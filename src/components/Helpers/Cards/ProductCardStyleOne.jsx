...
export default function ProductCardStyleOne({ datas }) {
	console.log(datas.price, 'datasdatas');
	...
	const quickViewHandler = (slug) => {
		...
		//     .then((res) => {
		//       console.log(res.data,"productsss")
		...
		>
			...
			{quickViewModal && (
				<div className='quicke-view-wrapper w-full h-full flex fixed left-0 top-0 justify-center z-50 items-center '>
					...
					></div>
					{/* New Modal */}
					<div className='max-full bg-white z-10 mx-auto p-4'>
						<div className='flex flex-col md:flex-row gap-8'>
							...
							<div className='md:w-1/2'>
								<span className='text-sm text-gray-500'>VEGETABLE</span>
								<h1 className='text-2xl font-bold mt-2'>{datas.productName}</h1>
								...
								<p className='text-3xl font-bold mt-4'>₹{datas.price}</p>

								<p className='mt-4 text-gray-600'>{datas.desc} </p>
								<button className='text-blue-600 mt-2'>See More</button>

								<div className='mt-6'>
									<p className='text-green-600'>
										Availability : {datas.quantity} Products Available
									</p>
									...
									<div className='flex items-center mt-4'>
										...
										/>
										<button className='border px-4 py-2'>+</button>
										<button className='ml-4 border p-2 rounded-full'>
											{/* Add heart icon */}
										</button>
									</div>
